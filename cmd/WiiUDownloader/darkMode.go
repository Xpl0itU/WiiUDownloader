...
func isDarkMode() bool {
	switch runtime.GOOS {
	case "darwin":
		cmd := newCommand("defaults", "read", "-g", "AppleInterfaceStyle")
	...
	case "windows":
		cmd := newCommand("reg", "query", "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Themes\\Personalize", "/v", "AppsUseLightTheme")
		output, err := cmd.Output()
	...
	case "linux":
		cmd := newCommand("gsettings", "get", "org.gnome.desktop.interface", "gtk-theme")
		output, err := cmd.Output()
